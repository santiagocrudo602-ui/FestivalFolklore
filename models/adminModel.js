...
const AdminModel = {
    crearNoche: async (nocheData) => {
        ...
        const { fecha, hora_inicio } = nocheData;
        const query = `
            INSERT INTO NOCHE (numero_noche, fecha, hora_inicio, id_festival)
            VALUES (?, ?, ?, ?)
        `;
        const [result] = await db.query(query, [nextNumeroNoche, fecha, hora_inicio, id_festival]);
        
        return { insertId: result.insertId, numero_noche: nextNumeroNoche };
    },
    editarNoche: async (id_noche, nocheData) => {
        const { fecha, hora_inicio } = nocheData;
        const query = `
            UPDATE NOCHE 
            SET fecha = ?, hora_inicio = ?
            WHERE id_noche = ?
        `;
        const [result] = await db.query(query, [fecha, hora_inicio, id_noche]);
